...
export function getDefaultToolbarItems() {
  ...
  toolbarItems.push({
    ...
    async onPress() {
      ...
      const mimeType = buffer.mimeType || "application/pdf";
      const extension = buffer.extension || "pdf";
      const blob = new Blob([buffer], { type: mimeType });
      if (navigator.msSaveOrOpenBlob) {
        navigator.msSaveOrOpenBlob(blob, "download." + extension);
      } else if (!supportsDownloadAttribute) {
        const reader = new FileReader();
        reader.onloadend = () => {
          const dataUrl = reader.result;
          downloadDocument(dataUrl, extension);
        };

        reader.readAsDataURL(blob);
      } else {
        const objectUrl = window.URL.createObjectURL(blob);
        downloadDocument(objectUrl, extension);
...
function downloadDocument(blob, extension) {
