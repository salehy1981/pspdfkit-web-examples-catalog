...
export async function attachListeners(instance, buttons) {
  ...
  instance.addEventListener("annotations.create", createdAnnotations => {
    // Enable the "Finish Signing" button if an ink signature has been created
    moveAnnotation(createdAnnotations.get(0), instance);
    updateToolbarItems(createdAnnotations, true, instance, buttons);
  });

  instance.addEventListener("annotations.delete", deletedAnnotations => {
    // Disable the "Finish Signing" button if an ink signature has been deleted
...
// When the document doesn't have any ink signature yet, we want to
// disable the "Finish signing" button. Once a signature is placed, it
// the ink signature button should be disabled and the "Finish signing"
...
async function updateToolbarItems(
  ...
  // When the document is loaded and when a ink signature annotation is
  ...
  const additionalButtons =
    signatures.status === "not_signed"
      ...
      : [{ type: "ink-signature", disabled: true }, resetButton, saveButton];
...
function moveAnnotation(annotation, instance) {
